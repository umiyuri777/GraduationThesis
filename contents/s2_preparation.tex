...
オープンワールドゲームに対するファジングを適用した手法としてBiFuzzという手法が存在する．BiFuzzでは，大域的ファジングと局所的ファジングの2段階でのファジングを行うことが特徴である．オープンワールドゲームでは状態空間が膨大になってしまう問題があるが，2段階のファジングによって人間の行動を再現するように入力列を生成することで，状態空間を限定し，オープンワールドゲームでのファジングを可能にした．
