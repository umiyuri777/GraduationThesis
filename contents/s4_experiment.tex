...
今回の実験にはパラメータを4つ使用する．BiFuzzのテストケース生成に使用されるの3つのパラメータをすべて使用する．

\begin{tabular}{ll} % l=left align (左寄せ)
    \textbf{cpNum:} & 追加する寄り道の数を決定する \\
    \textbf{cpNum\_range:} & 経由地の生成範囲 \\
    \textbf{cpNum\_dir:} & 経由地の生成方向 \\
    \textbf{tree:} & 障害物となる木の数 \\
    \label{tb:bifuzz_pram}
\end{tabular}
