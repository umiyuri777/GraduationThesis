...
\figref{fig:testflow}に本実験の流れを示す．本実験では，BiFuzzに対して様々なパラメータについて5回ずつテストを実行し，不具合発見の成否を実験結果として記録する．
収集したデータに対し，提案手法の有効性を評価するため，以下の3つの項目に関して予測モデルの構築を試みる．
\begin{itemize}
    \item \testfirst
    \item \testsecond
    \item \testthird
\end{itemize}
これらの予測モデルを構築するために，以下の3つの回帰分析アルゴリズムを採用した．

\begin{description}
 \item[ロジスティック回帰分析]
  事象の発生確率を予測する一般化線形モデルの一種である．説明変数と目的変数の関係をシグモイド関数を用いてモデル化し，結果を0から1の確率として出力する．各変数の偏回帰係数を確認することで，どのパラメータが予測に寄与しているかを定量的に評価しやすい特徴がある．

 \item[決定木]
  データを特定のルールに基づいて分割し，木構造のモデルを作成する手法である．モデルの構造が直感的であり，人間にとって解釈が容易である．また，各特徴量がデータの分割にどれだけ寄与したかを示す重要度を算出することができる．

 \item[ランダムフォレスト]
  複数の決定木を構築し，それらの結果を統合して予測を行うアンサンブル学習手法である．単一の決定木と比較して過学習を起こしにくく，高い予測精度と汎用性が期待できる．決定木と同様に，特徴量の重要度を算出できる．
\end{description}

構築した各モデルの性能を評価するために，データセットを用いた10分割交差検証を行う．本実験では評価指標として以下の4つを用いる．なお，各指標は正解データ（実際に不具合が発見されたケース）と予測結果の整合性に基づいて算出される．

\begin{description}
 \item[適合率 (Precision)]
  モデルが「不具合を発見する（正例）」と予測したデータのうち，実際に不具合が発見された割合である．この値が高いほど，不具合がないのに発見すると予測するような誤検知が少ないことを示す．
  \[
  \text{Precision} = \frac{TP}{TP + FP}
  \]
  ここで，$TP$ (True Positive) は真陽性，$FP$ (False Positive) は偽陽性を表す．

 \item[再現率 (Recall)]
  実際に不具合が発見されたデータのうち，モデルが正しく「不具合を発見する」と予測できた割合である．この値が高いほど，不具合の見落としが少ないことを示す．
  \[
  \text{Recall} = \frac{TP}{TP + FN}
  \]
  ここで，$FN$ (False Negative) は偽陰性を表す．

 \item[F値 (F-measure)]
  適合率と再現率の調和平均である．適合率と再現率はトレードオフの関係にあることが多いため，両者のバランスを総合的に評価するために用いられる．
  \[
  \text{F-measure} = \frac{2 \cdot \text{Precision} \cdot \text{Recall}}{\text{Precision} + \text{Recall}}
  \]

 \item[AUC (Area Under the Curve)]
  ROC曲線下の面積を表す指標である．ROC曲線とは，判定の閾値を変化させた際の真陽性率を縦軸，偽陽性率を横軸にとり，その関係をプロットしたものである．AUCは0から1の値をとり，1に近いほどモデルの判別性能が高いことを示す．閾値に依存しないモデルの性能評価が可能である．
\end{description}

また，不具合発見の予測の正確性を比較するため，「すべてのテストケースで不具合を発見すると予測する」ベースラインを用意し，提案手法と比較を行う．

なお，本実験における予測モデルの構築，ハイパーパラメータの最適化，および10分割交差検証の実装には，Python 3.13.3 と scikit-learn 1.7.2 を使用した．
また，各アルゴリズムのハイパーパラメータについては，scikit-learnのGridSearchCVを用いてグリッドサーチを行い，F値が最大となる値を採用した．


\begin{figure}[p]
    \begin{center}
        % \hspace{-23mm}
        \includegraphics[height=0.95\textheight, keepaspectratio]{img/test_method_3.pdf}
        \caption{実験の概要図}
        \label{fig:testflow}
    \end{center}
\end{figure}
...
また，BiFuzz の実行においては，1回の試行につき実行時間が30分超過した場合をタイムアウトとして設定した．

今回の実験では\cref{fig:stack_bug_object}のようなスタックする不具合を模したオブジェクトをマップ上に100箇所ランダムに配置する．このオブジェクトは外側からかは侵入可能であるが，内側からは脱出不可能になっている．一定時間このオブジェクトの内側にいると「スタックした」と判定される．

\subsubsection*{対象ゲーム}実験の対象となるビデオゲームとして，BiFuzzの提案論文\cite{Kato2024}でも使用された「STAR COLLECTION」を採用した．本ゲームは ゲームエンジンであるUnity\footnote{\url{https://unity.com/ja}}を用いて開発された，簡易的なオープンワールドゲームである．ゲームの目的は，広大なマップ上に配置された星のアイテムをすべて回収することである．ゲーム内の環境はシンプルに構成されており，マップ上には\cref{fig:player}のようなプレイヤキャラクタ，\cref{fig:star}のような回収対象となる星のアイテム，そして\cref{fig:tree}のような移動の障害物となる木のオブジェクトのみが存在する．

\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{img/stack_1.png}
    \caption{スタックするオブジェクト}
  ...
  \caption{StarCollectionに配置されているオブジェクト}
\end{figure}
...
\begin{split}
    ...
    P &= \frac{1}{1 + \exp(-z)} \\
    \text{where, } z &= 0.445 + 0.060 tree_{std} + 0.439 cpNum_{std} \\
      &\quad + 1.316 cpNum\_range_{std} + 0.046 cpNum\_dir_{std}
\end{split}
\end{equation}
...
次に，決定木およびランダムフォレストの10分割交差検証による特徴量重要度を表\ref{tab:dt_single_importance}および表\ref{tab:rf_single_importance}に示す．
両モデルとも共通して $cpNum\_range$ が約69\%〜74\%と最も高い重要度を示し，次いで $cpNum$ が約26\%〜31\%の重要度を示した．これら2つのパラメータで重要度全体の99\%以上を占めており，$tree$ や $cpNum\_dir$ の影響は無視できるほど小さい．

以上の結果から，使用するアルゴリズムに関わらず，$cpNum\_range$ と  $cpNum$の経路生成に関するパラメータが不具合発見に大きく影響していることが示された．また，$tree$ が不具合発見に与える影響が少なかったことから，マップ内の障害物数による影響は極めて小さいとする共通した傾向が明らかになった．
...
この要因として，表\ref{data_set_2}に示したデータセットの不均衡が挙げられる．「5回のうち1回以上不具合を発見する」条件を満たす正例は全体の約84\%を占めており，学習されたモデルは特徴量に関わらず不具合発見と予測する傾向が強まったため，ベースラインとの差異が生じなかったと考えられる．
...
特に決定木の再現率に着目すると0.9882と高い値を示している．これは，5回とも不具合を発見できる再現性の高いパラメータ設定を，モデルが高い精度で予測できていることを意味する．
...
以上のことから，データの偏りが少ない条件下においては，提案手法によって有効な予測モデルが構築可能であると結論付ける．
...
この結果は，本実験で対象としたスタックする不具合に関して，マップ上の障害物密度の変化は発見率に大きな影響を与えないことを示唆している．
提案手法における適用プロセスは，ゲームになんらかの変更が加わった際に実施されることを想定している．この変更にはマップ構造の変化も含まれるが，本実験の結果より，マップ構造の変化は不具合発見に大きな影響を与えないことが確認された．したがって，マップ構造が変化した場合であっても，経路生成に関するパラメータを重点的に探索・調整する提案手法の方針は妥当であり，その適用プロセスの有効性が示されたといえる．
