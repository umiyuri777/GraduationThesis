...
式(\ref{eq:logistic_multi_split})の各変数の係数に着目すると，$cpNum\_dir$の係数が0.190446と最も大きく，次いで$cpNum\_range$が正の値を示している．1回の実行ごとの分析（式(\ref{eq:logistic_single_split})）と比較して，$cpNum\_dir$の影響度が相対的に大きくなっていることが確認できる．

表\ref{tab:logistic_multi_metrics}に，10分割交差検証によるモデルの評価結果を示す．
F値の平均は0.9157であり，表\ref{tab:baseline_result_5}に示したベースラインのF値0.9159とほぼ同等の結果となった．また，再現率が1.0000であることから，本モデルはすべてのテストケースに対して「不具合を発見する」と予測する傾向が極めて強いことが示唆される．
...
結果より，$cpNum\_range$ の重要度が約77\%と最も高く，次いで $cpNum$ が約22\%を占めている．これら2つのパラメータで全体の重要度の99\%以上を占めている点は\cref{chap:1kai}と同様の傾向である．
...
以上の結果より，本実験設定（5回の実行のうち1回以上不具合発見）においては，ロジスティック回帰分析，決定木，ランダムフォレストのいずれの手法を用いても，ベースラインと比較して予測性能の向上は見られなかった．
この要因として，実験データの偏りが挙げられる．本実験で収集したデータにおいて，「5回のうち1回以上不具合を発見する」という条件を満たす正例（不具合発見あり）は全体の約75\%（Accuracyにおける0.8449相当）を占めていた．このように正例の割合が極めて高い不均衡なデータセットであったため，学習されたモデルは特徴量に関わらず「不具合発見」と予測する傾向が強まり，その結果，すべてを正と予測するベースライン手法と予測結果に差異が生じなかったと考えられる．


\subsection{5回の実行のうち5回とも不具合発見するか}\label{chap:test3}
本節では，BiFuzzのパラメータ設定ごとに5回実行を行った際，5回すべての実行において不具合が発見されたか否かを予測対象とした場合の結果について述べる．
...
式(\ref{eq:logistic_all_split})の各変数の係数に着目すると，$cpNum\_range$ の係数が 0.035818 と最も大きく，不具合発見確率への寄与が最も大きいことがわかる．

表\ref{tab:logistic_all_metrics}に，10分割交差検証によるモデルの評価結果を示す．
F値の平均は 0.6648 となった．適合率が 0.5949 と比較的低い値に留まっており，ロジスティック回帰分析では，高確率で不具合を発見できるケースの絞り込みにおいて，誤検知が多く発生していることが示唆される．

\begin{table}[H]
    \caption{ロジスティック回帰分析の評価結果（5回の実行のうち5回とも不具合発見）}
...
$cpNum\_range$ の重要度が約65\%，$cpNum$ が約33\%を占め，\cref{chap:test1}や\cref{chap:test2}と同様にこれら2つの特徴量が支配的であることが確認された．また，$tree$ の重要度は約1.4\%であり，マップ構造の違いが結果に与える影響は軽微であると言える結果も\cref{chap:test1}や\cref{chap:test2}と同様であった．

\begin{table}[H]
    \caption{決定木における特徴量重要度（5回の実行のうち5回とも不具合発見）}
...
F値の平均は 0.7769 となり，ロジスティック回帰分析（0.6648）と比較して大幅な精度の向上が見られた．特に再現率（Recall）が 0.9882 と極めて高い値を示している．これは，5回連続で不具合を発見できるような有効なパラメータ設定に対して，決定木モデルがほぼ見逃すことなく「発見できる」と予測できていることを示している．

\begin{table}[H]
    \caption{決定木の評価結果（5回の実行のうち5回とも不具合発見）}
...
決定木と同様に $cpNum\_range$（約63\%）と $cpNum$（約32\%）が高い重要度を示している．$cpNum\_dir$ と $tree$ についてはそれぞれ数\%程度であり，影響度は限定的であった．

\begin{table}[H]
    \caption{ランダムフォレストにおける特徴量重要度（5回の実行のうち5回とも不具合発見）}
...
F値の平均は 0.7738 であり，決定木とほぼ同等の性能を示した．再現率は 0.9659 と高い水準を維持しており，決定木同様，有効なパラメータ設定の見逃し（False Negative）を低く抑えられている．一方で適合率は 0.6466 であり，実際には5回連続で成功しないケースを成功と予測してしまう誤検知が一定数含まれる傾向にある．

\begin{table}[H]
    \caption{ランダムフォレストの評価結果（5回の実行のうち5回とも不具合発見）}
...
\textbf{RQ1: 不具合発見とマップ構造は関係しているか}

すべての実験設定（1回の実行，1回以上の成功，5回連続の成功）およびすべての分析手法において，$tree$（障害物となる木の数）の特徴量重要度および回帰係数は，他のパラメータと比較して著しく低い値となった．
例えば，決定木を用いた分析においては，$tree$ の重要度は最大でも 1.41\%（表\ref{tab:dt_all_importance}）に留まっており，予測モデルへの寄与は極めて小さい．
この結果から，本実験で対象とした「特定のオブジェクトにスタックする不具合」に関しては，マップ全体の障害物密度（マップ構造）は不具合発見の成否に大きな影響を与えていないと考えられる．
不具合発見に支配的な影響を与えているのは，プレイヤキャラクタの移動経路を決定する $cpNum\_range$ および $cpNum$ であり，マップの環境要因よりも，探索のエージェントがどのように移動するかという動作パラメータの方が重要であると言える．

\textbf{RQ2: 提案手法によって不具合発見を予測できる有効なモデルは構築できるか}

本実験では，異なる3つの粒度で予測モデルの構築を試みた．
「1回の実行で不具合発見するか」という予測（4.1節）においては，決定木およびランダムフォレストを用いることで，ベースライン（すべて成功と予測する場合）と比較してF値を約0.13向上させることができた．これは，提案手法が個々のテスト実行の成否を一定の精度で予測可能であることを示している．

「5回の実行のうち1回以上不具合発見するか」という予測（4.2節）においては，正例（不具合発見あり）の割合が全体の約84\%を占める不均衡なデータセットとなったため，構築されたモデルはベースラインと同等の性能に留まった．これは，BiFuzzが多くのパラメータ設定において「少なくとも1回は」不具合を発見できる高い探索能力を持っていることの裏返しでもあるが，効率化のための選別という観点では予測の恩恵が小さいケースであると言える．

「5回の実行のうち5回とも不具合発見するか」という予測（4.3節）においては，決定木およびランダムフォレストが高い再現率（約0.97〜0.99）を達成した．
これは，再現性高く不具合を発見できる「質の高いパラメータ設定」を，モデルが見逃すことなく抽出できていることを意味する．
ファジングにおいて，不具合を確実に再現できるパラメータを事前に特定できることは，修正後の回帰テストの効率化において極めて重要である．適合率が約64\%であるため，選別されたパラメータの中に実際には5回連続で成功しないものが含まれる可能性はあるものの，すべてのパラメータを試行する場合と比較すれば，実行すべきテストケース数を大幅に削減しつつ，重要なパラメータを網羅的に実行することが可能となる．

以上より，提案手法を用いることで，特に不具合発見の確実性が高いパラメータ設定を予測・選別する有効なモデルが構築できると結論付けられる．
