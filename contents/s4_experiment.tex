...
\begin{itemize}
    ...
    \item \testthird
\end{itemize}
これらの予測モデルを構築するために，以下の3つの機械学習アルゴリズムを採用した．

\begin{description}
 \item[ロジスティック回帰分析]
  事象の発生確率を予測する一般化線形モデルの一種である．説明変数と目的変数の関係をシグモイド関数を用いてモデル化し，結果を0から1の確率として出力する．各変数の偏回帰係数を確認することで，どのパラメータが予測に寄与しているかを定量的に評価しやすいという特徴がある．
...
\begin{description}
 ...
 \item[AUC (Area Under the Curve)]
  ROC曲線（受信者操作特性曲線）下の面積を表す指標である．0から1の値をとり，1に近いほどモデルの判別性能が高いことを示す．閾値に依存しないモデルの性能評価が可能である．
\end{description}

また，不具合発見の予測の正確性を比較するため，「すべてのテストケースで不具合を発見すると予測する」というベースラインを用意し，提案手法と比較を行う．

なお，本実験における予測モデルの構築，ハイパーパラメータの最適化，および十分割交差検証の実装には，Python 3.13.3 と機械学習ライブラリ scikit-learn 1.7.2 を使用した．
...
本実験では，パラメータの組み合わせと不具合発見の関係性を分析するために，以下の3つの機械学習アルゴリズムを採用した．
...
\begin{description}
 ...
 \item[ランダムフォレスト]
  複数の決定木を構築し，それらの結果を統合（多数決や平均化）して予測を行うアンサンブル学習手法である．単一の決定木と比較して過学習を起こしにくく，高い予測精度と汎用性が期待できる．決定木と同様に，特徴量の重要度を算出可能である．
\end{description}

また，構築した各モデルの性能を定量的に評価するために，以下の4つの指標を用いる．なお，本実験において各指標は，正解データ（実際に不具合が発見されたケース）と予測結果の整合性に基づいて算出される．

\begin{description}
 \item[適合率]
  モデルが「不具合を発見する（正例）」と予測したデータのうち，実際に不具合が発見された割合である．この値が高いほど，誤検知（不具合がないのに発見すると予測するミス）が少ないことを示す．
  \[
  \text{Precision} = \frac{TP}{TP + FP}
  \]
  ここで，$TP$ (True Positive) は真陽性，$FP$ (False Positive) は偽陽性を表す．
...
\begin{figure}[p]
    \begin{center}
        \hspace{-23mm}
        \includegraphics[height=0.9\textheight, keepaspectratio]{img/test_method_3.pdf}
        \caption{実験の概要図}
        \label{fig:testflow}
    \end{center}
\end{figure}
