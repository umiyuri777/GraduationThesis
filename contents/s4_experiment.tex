...
\subsection{1回の実行で不具合発見するか}\label{chap:1kai}
...
\subsection{5回の実行のうち1回以上不具合発見するか}
...
\begin{table}[H]
  \centering
  \caption{ベースラインの評価結果 (5回の実行のうち1回以上)}
  \label{tab:baseline_result_5}
  \begin{tabular}{l|r} \hline
    評価指標 & 値 \\ \hline \hline
    Accuracy (正解率) & 0.8449 \\
    Precision (適合率) & 0.8449 \\
    Recall (再現率) & 1.0000 \\
    F1 Score (F値) & 0.9159 \\ \hline
  \end{tabular}
\end{table}
...
F値の平均は0.9157であり，表\ref{tab:baseline_result_5}に示したベースラインのF値0.9159とほぼ同等の結果となった．また，再現率（Recall）が1.0000であることから，本モデルはすべてのテストケースに対して「不具合を発見する」と予測する傾向が極めて強いことが示唆される．

\begin{table}[H]
    \caption{ロジスティック回帰分析の評価結果（5回の実行のうち1回以上）}
    \label{tab:logistic_multi_metrics}
    \centering
    \begin{tabular}{lcccc}
        \hline
        評価指標 & 平均値 & 標準偏差 & 最小値 & 最大値 \\\hline 
        Accuracy (正解率) & 0.8444 & 0.0023 & 0.8426 & 0.8472 \\\hline
        Precision (適合率) & 0.8444 & 0.0023 & 0.8426 & 0.8472 \\\hline
        Recall (再現率) & 1.0000 & 0.0000 & 1.0000 & 1.0000 \\\hline
        F1 Score (F値) & 0.9157 & 0.0013 & 0.9146 & 0.9173 \\\hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    \caption{決定木における特徴量重要度（5回の実行のうち1回以上）}
...
$cpNum\_range$ が約72\%，$cpNum$ が約24\%と，決定木と同様の傾向を示しているが，$tree$ や $cpNum\_dir$ にもわずかながら重要度が割り振られている点が異なる．

\begin{table}[H]
    \caption{ランダムフォレストにおける特徴量重要度（5回の実行のうち1回以上）}
...
F値の平均は0.9145であり，ロジスティック回帰分析や決定木と同様に，ベースライン（0.9159）と極めて近い値を示した．再現率は0.9973とわずかに1.0を下回ったものの，ほぼすべてのケースで不具合発見を予測している傾向にある．

\begin{table}[H]
    \caption{ランダムフォレストの評価結果（5回の実行のうち1回以上）}
    \label{tab:rf_multi_metrics}
    \centering
    \begin{tabular}{lcccc}\hline
        評価指標 & 平均値 & 標準偏差 & 最小値 & 最大値 \\\hline
        Accuracy (正解率) & 0.8426 & 0.0041 & 0.8333 & 0.8472 \\\hline
        Precision (適合率) & 0.8445 & 0.0021 & 0.8426 & 0.8472 \\\hline
        Recall (再現率) & 0.9973 & 0.0056 & 0.9835 & 1.0000 \\\hline
        F1 Score (F値) & 0.9145 & 0.0025 & 0.9086 & 0.9173 \\\hline
    \end{tabular}
\end{table}
